/*
  This program is Zuul, a game where you go through rooms and pick up items
    Map reference info from geeksforgeeks.org/map-associative-containers-the-c-standard-template-library-stl/
    Map incorporation & move funct done with help from Omar Nassar
 */
...
//initializing functions (to better organize code)
...
int main() {
  ...
  //program
  cout << "Welcome to my Zuul! Here is the story: " << endl;
  cout << "You are running late for work. Go through your house" << endl;
  cout << "and pick up the appropriate items needed for your day." << endl;
  ...
  while (run) {
	...
	for (int c = 0; c < inventory.size(); c++) {
	  if (currentRoom == 1 && inventory[a] == 1 && inventory[b] == 2 && inventory[c] == 3) {
	    cout << endl << "You won my Zuul! GGs" << endl << endl;
	    return 0;
	  }
	}
      }
	...
	cout << endl << "Do you really need this item?" << endl;
...
void initializeRooms(vector<Room*>* rooms) {
  ...
  living -> setDescription((char*)("in the living room, the biggest room of the house."));
  living -> setId(2);
  ...
  inhall -> setDescription((char*)("in the interior hallway."));
  inhall -> setId(8);
  ...
  office -> setDescription((char*)("in your office."));
  office -> setId(9);
  ...
  outside -> setDescription((char*)("in the outside garden. No exit!"));
  outside -> setId(11);
...
void initializeItems(vector<Item*>* items) {
  ...
  remote -> setName((char*)("remote"));
  remote -> setId(4);
  items -> push_back(remote);
  Item* tbrush = new Item();
  tbrush -> setName((char*)("toothbrush"));
  tbrush -> setId(5);
...
void getItem(vector<Room*>* rooms, vector<Item*>* items,vector<int>* invtry, int currentRoom, char name[]) {
  ...
  for (r = rooms->begin(); r != rooms->end(); r++) {
    if (currentRoom == (*r) -> getId()) {
      for (i = items->begin(); i != items->end(); i++) {
	//if item is in the room (thanks Omar)
